...
@section('content')
    <div class="card shadow mt-3">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h3 class="text-primary">Edit {{ $customer->name }}</h3>
        </div>
        <div class="card-body">
            <form action="{{ route('customers.update', ['id' => $customer->id]) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('name') is-invalid @enderror" name="name"
                        id="name" placeholder="Enter the customer name" value="{{ $customer->name }}">
                    @error('name')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" inputmode="numeric" class="form-control @error('phone') is-invalid @enderror"
                        ...
                        onkeypress="return isNumberKey(event)" value="{{ $customer->phone }}">
                    @error('phone')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
